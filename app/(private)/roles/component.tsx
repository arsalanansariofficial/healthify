...
import { deletePharmaSalt, deletePharmaSalts, updateRole } from '@/lib/actions';
...
export function TableCellViewer(props: { item: Role }) {
  ...
  const { handleSubmit } = useHookForm(
    handler,
    updateRole.bind(null, props.item.id) as (data: unknown) => Promise<unknown>
  );
...
export default function Component(props: { user: User; roles: Role[] }) {
  return (
      ...
      {hasPermission(props.user.permissions, 'view:users') && (
        <DataTable
          button={
            <>
              <Button asChild>
                <Link href='/roles/add'>Add</Link>
              </Button>
              <Button asChild variant='secondary'>
                <Link href='/roles/assign'>Assign</Link>
              </Button>
            </>
          }
          columns={
